...
def export(ui=None, renpy=None, pygame=None, name=""):
    ...
    class Item(object):
        ...
        def springback(self, tb):
            ...
            self.offset = Springback(self, tb, x + ox, y + oy)
    ...
    class ToolBar(renpy.Displayable):
        ...
        def _reset(self):
            for i in range(min(len(self.items), len(self.item_rects))):
                self.items[i].rect = self.item_rects[i]
                self.items[i].offset = Fixed(0, 0)
        ...
        def event(self, ev, x, y, st):
            ...
            if ev.type == pygame.MOUSEMOTION or (ev.type == pygame.MOUSEBUTTONUP and ev.button == 1):
                ...
                if self.dragging and self.drag_item is not None:
                    ...
                    self.drag_item.offset = Fixed(dx, dy)
                    renpy.redraw(self, 0)
            ...
            if ev.type == pygame.MOUSEBUTTONUP and ev.button == 1:
                ...
                if self.dragging and self.drag_item is not None:
                    ...
                    for areaeven in self.areaevens:
                        print("pos: ({}, {})".format(
                            *self.drag_item.center_pos()))
                        print("even rect: ({}, {}, {}, {})".format(*areaeven.rect))
                        if pos_area_in(self.drag_item.center_pos(), areaeven.rect):
